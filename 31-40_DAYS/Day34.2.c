...
int main() {
    int size;
    int i;
    int max1; // Stores the largest element
    int max2; // Stores the second largest element
    ...
    for (i = 2; i < size; i++) {
        ...
        if (arr[i] > max1) {
            max2 = max1; // Demote old max1 to max2
            max1 = arr[i]; // New largest
        }
        
        // Case 2: Current element is between max1 and max2
        else if (arr[i] > max2 && arr[i] != max1) {
            max2 = arr[i]; // New second largest
        }
    }
